...
export type WordSurfaceState = {
  highlight: WordRange | null;
  loading: WordRange | null;
  phraseRange: WordRange | null;
};

const idleSurfaceState: WordSurfaceState = {
  highlight: null,
  loading: null,
  phraseRange: null,
};

export function useWordSurfacePronunciation(enabled: boolean) {
  ...
  const getSurfaceState = useCallback(
    (surfaceKey: string): WordSurfaceState => {

...
## Motif extension theme (shadcn/ui)

Motif’s in-extension UI uses [shadcn/ui](https://ui.shadcn.com/) primitives themed from this document.

### Typography in Motif

| DESIGN.md token | Motif implementation |
|-----------------|-------------------|
| Display / body sans | System UI stack (`system-ui`, Helvetica Neue, Arial) |
| Code / shortcuts | System monospace stack |

CursorGothic from the marketing spec is not bundled in the extension.

### shadcn CSS variable map

Defined in [`assets/styles/globals.css`](assets/styles/globals.css):

| DESIGN.md | shadcn variable |
|-----------|-----------------|
| `canvas` | `--background` |
| `surface-card` | `--card` |
| `ink` | `--foreground` |
...
| `rounded.md` / `rounded.lg` | `--radius` (8px) / `rounded-lg` (12px) |

Cards use **hairline borders only** — no drop shadows (override default shadcn elevation).

### Where it applies today

- **Options page** (`entrypoints/options/`) — React + shadcn (`Card`, `Button`, `Select`, `Progress`, `Badge`, etc.)
- **Overlays** (TTS, transcript, word) — still on legacy CSS in shadow DOM; see Phase 2 below.

### Phase 2 — overlay migration (planned)

Floating overlays run in shadow DOM ([`components/overlay/mount-shadow-react.tsx`](components/overlay/mount-shadow-react.tsx)). Next step:

1. Build a Tailwind bundle (`ui.css?inline`) for shadow injection
2. Migrate overlay components incrementally to shadcn primitives
3. Keep compact panel constraints (320–360px width, draggable header)

Do **not** use dashboard/sidebar [blocks](https://ui.shadcn.com/blocks) for overlays — use compact `Card` / `Button` patterns only.
